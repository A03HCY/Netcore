# Netcore Documentation

Welcome to the Netcore documentation. This documentation is also available online at [https://netcore.acdp.top](https://netcore.acdp.top).

## Contents

### Getting Started
- [Installation](getting_started/installation.md)
- [Quick Start](getting_started/quickstart.md)
- [Basic Examples](getting_started/examples.md)

### Architecture
- [Architecture Overview](architecture/overview.md)
- [LSO Protocol Layer](architecture/lso_protocol.md)
- [Transport Layer (Pipe)](architecture/pipe.md)
- [Application Layer (Endpoint)](architecture/endpoint.md)

### Advanced Usage
- [Custom Transport Implementation](advanced/custom_transport.md)
- [Large Data Transfer](advanced/large_data.md)
- [Error Handling](advanced/error_handling.md)

### API Reference
- [LSO Protocol API](api/lso.md)
- [Pipe API](api/pipe.md)
- [Endpoint API](api/endpoint.md) 